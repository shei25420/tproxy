...
nst_nosql_handler(hpx_appctx_t *appctx) {
    ...
    switch(appctx->st0) {
        ...
        case NST_NOSQL_APPCTX_STATE_HIT_MEMORY:
            ...
            nst_memory_obj_detach(&tproxy.nosql->store.memory, appctx->ctx.tproxy.store.memory.obj);
        ...
        case NST_NOSQL_APPCTX_STATE_HIT_DISK:
            {
                ...
                switch(appctx->st1) {
                    case NST_DISK_APPLET_HEADER:
                        ...
                        offset += ret;
                        appctx->ctx.tproxy.store.disk.offset += ret;
...
nst_nosql_housekeeping() {
    nst_dict_t   *dict  = &tproxy.nosql->dict;
    nst_store_t  *store = &tproxy.nosql->store;
    uint64_t      start;
    ...
    if(global.tproxy.nosql.status == NST_STATUS_ON && master == 1) {
        int  dict_cleaner = global.tproxy.nosql.dict_cleaner;
        int  data_cleaner = global.tproxy.nosql.data_cleaner;
        int  disk_cleaner = global.tproxy.nosql.disk_cleaner;
        int  disk_saver   = global.tproxy.nosql.disk_saver;
        int  ms           = 10;
        int  ratio        = 1;

#ifndef USE_THREAD
        int  disk_loader  = global.tproxy.nosql.disk_loader;
#endif
        ...
        while(store->disk.loaded && disk_saver--) {
            nst_store_memory_sync_disk(tproxy.nosql);
...
nst_nosql_init() {
    ...
    root       = global.tproxy.nosql.root;
    dict_size  = global.tproxy.nosql.dict_size;
    data_size  = global.tproxy.nosql.data_size;
    size       = dict_size + data_size;
    clean_temp = global.tproxy.nosql.clean_temp;

    tproxy.applet.nosql.fct = nst_nosql_handler;

    if(global.tproxy.nosql.status == NST_STATUS_ON) {

        shmem = nst_shmem_create("nosql.shm", size, global.tune.bufsize, NST_DEFAULT_CHUNK_SIZE);

        if(!shmem) {
            ha_alert("Failed to create tproxy nosql memory zone.\n");
            exit(1);
        }

        global.tproxy.nosql.shmem = shmem;

        if(nst_shctx_init(shmem) != NST_OK) {
            ha_alert("Failed to init tproxy nosql memory.\n");
            exit(1);
        }

        tproxy.nosql = nst_shmem_alloc(shmem, sizeof(nst_core_t));

        if(!tproxy.nosql) {
            ha_alert("Failed to init tproxy nosql core.\n");
            exit(1);
        }

        memset(tproxy.nosql, 0, sizeof(*tproxy.nosql));

        tproxy.nosql->shmem = shmem;
        tproxy.nosql->root  = root;

        if(nst_store_init(&tproxy.nosql->store, root, shmem, clean_temp, tproxy.nosql) != NST_OK) {
            ha_alert("Failed to init tproxy nosql store.\n");
            exit(1);
        }

        if(nst_dict_init(&tproxy.nosql->dict, &tproxy.nosql->store, shmem, dict_size) != NST_OK) {
            ha_alert("Failed to init tproxy nosql dict.\n");
            exit(1);
        }
...
nst_nosql_check_applet(hpx_stream_t *s, hpx_channel_t *req, hpx_proxy_t *px) {
    ...
    if(global.tproxy.nosql.status == NST_STATUS_ON && px->tproxy.mode == NST_MODE_NOSQL) {
        ...
        s->target = &tproxy.applet.nosql.obj_type;
...
_nst_nosql_create_header(hpx_stream_t *s, nst_http_txn_t *txn, nst_rule_prop_t *prop) {
    nst_memory_t        *mem  = &tproxy.nosql->store.memory;
...
err:
    nst_shmem_free(tproxy.nosql->shmem, item_sl);
    nst_shmem_free(tproxy.nosql->shmem, item_ct);
    nst_shmem_free(tproxy.nosql->shmem, item_te);
    nst_shmem_free(tproxy.nosql->shmem, item_eoh);
...
nst_nosql_create(hpx_stream_t *s, hpx_http_msg_t *msg, nst_ctx_t *ctx) {
    ...
    nst_dict_t         *dict   = &tproxy.nosql->dict;
    nst_memory_t       *mem    = &tproxy.nosql->store.memory;
    nst_disk_t         *disk   = &tproxy.nosql->store.disk;
    ...
    if(ctx->state == NST_CTX_STATE_CREATE || ctx->state == NST_CTX_STATE_UPDATE) {
        ...
        if(nst_store_memory_off(ctx->rule->prop.store)) {
            ...
            while(item) {
                ...
                nst_shmem_free(tproxy.nosql->shmem, t);
            }
        }
    }
...
nst_nosql_append(hpx_http_msg_t *msg, nst_ctx_t *ctx, unsigned int offset, unsigned int len) {
    ...
    blk    = htxret.blk;
    offset = htxret.ret;
    disk   = &tproxy.nosql->store.disk;
    mem    = &tproxy.nosql->store.memory;
...
nst_nosql_exists(nst_ctx_t *ctx) {
    ...
    if(ret == NST_CTX_STATE_INIT) {
        ...
        if(!nst_store_disk_off(ctx->rule->prop.store)) {
            ...
            if(!disk->loaded || global.tproxy.nosql.always_check_disk) {
...
nst_nosql_abort(nst_ctx_t *ctx) {
    ...
    if(entry->state == NST_DICT_ENTRY_STATE_INIT || entry->state == NST_DICT_ENTRY_STATE_UPDATE) {
        ...
        if(ctx->store.memory.obj) {
            nst_memory_obj_abort(&tproxy.nosql->store.memory, ctx->store.memory.obj);
        }

        if(ctx->store.disk.obj.file) {
            nst_disk_obj_abort(&tproxy.nosql->store.disk, &ctx->store.disk.obj);
        }
    }
...
nst_nosql_delete(nst_key_t *key) {
    ...
    if(entry) {
        ...
        if(entry->state == NST_DICT_ENTRY_STATE_VALID
            ...
            if(entry->store.memory.obj) {
                ...
                nst_memory_incr_invalid(&tproxy.nosql->store.memory);
            }

            if(entry->store.disk.file) {
                nst_disk_file_remove(entry->store.disk.file);
                nst_shmem_free(tproxy.nosql->shmem, entry->store.disk.file);
    ...
    if(!tproxy.nosql->store.disk.loaded && global.tproxy.nosql.root.len){
        nst_disk_obj_t  disk;
        hpx_buffer_t    *buf = get_trash_chunk();

        disk.file = buf->area;

        ret = nst_disk_purge_by_key(&disk, key, global.tproxy.nosql.root);
    }
