/*
 * tproxy nosql filter related variables and functions.
 *
 *
 *
...
_nst_nosql_filter_attach(hpx_stream_t *s, hpx_filter_t *filter) {
    ...
    if(global.tproxy.nosql.status != NST_STATUS_ON || conf->status != NST_STATUS_ON) {
        return 0;
    }
    ...
    if(!filter->ctx) {
        ...
        rule_cnt = tproxy.proxy[conf->pid]->rule_cnt;
        key_cnt  = tproxy.proxy[conf->pid]->key_cnt;
...
_nst_nosql_filter_http_headers(hpx_stream_t *s, hpx_filter_t *filter, hpx_http_msg_t *msg) {
    ...
    if(ctx->state == NST_CTX_STATE_INIT) {
        ...
        ctx->rule = tproxy.proxy[px->uuid]->rule;
