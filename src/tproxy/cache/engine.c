...
_nst_cache_memory_handler(hpx_appctx_t *appctx) {
    ...
    if(res->flags & (CF_SHUTW|CF_SHUTR|CF_SHUTW_NOW)) {
        appctx->ctx.tproxy.store.memory.item = NULL;
    }

    if(appctx->ctx.tproxy.store.memory.item) {
        item = appctx->ctx.tproxy.store.memory.item;
...
out:
    appctx->ctx.tproxy.store.memory.item = item;
...
_nst_cache_disk_handler(hpx_appctx_t *appctx) {
    ...
    header_len  = appctx->ctx.tproxy.store.disk.header_len;
    payload_len = appctx->ctx.tproxy.store.disk.payload_len;
    offset      = appctx->ctx.tproxy.store.disk.offset;
    fd          = appctx->ctx.tproxy.store.disk.fd;
    res_htx     = htxbuf(&res->buf);
    total       = res_htx->data;
    ...
    switch(appctx->st1) {
        case NST_DISK_APPLET_HEADER:
            ...
            appctx->ctx.tproxy.store.disk.offset = offset;
        ...
        case NST_DISK_APPLET_PAYLOAD:
            ...
            if(appctx->ctx.tproxy.store.disk.payload_len == 0) {
                ...
                appctx->ctx.tproxy.store.disk.payload_len -= ret;
...
nst_cache_housekeeping() {
    nst_dict_t   *dict  = &tproxy.cache->dict;
    nst_store_t  *store = &tproxy.cache->store;
    uint64_t      start;
    ...
    if(global.tproxy.cache.status == NST_STATUS_ON && master == 1) {
        int  dict_cleaner = global.tproxy.cache.dict_cleaner;
        int  data_cleaner = global.tproxy.cache.data_cleaner;
        int  disk_cleaner = global.tproxy.cache.disk_cleaner;
        int  disk_saver   = global.tproxy.cache.disk_saver;
        int  ms           = 10;
        int  ratio        = 1;

#ifndef USE_THREAD
        int  disk_loader  = global.tproxy.cache.disk_loader;
#endif
        ...
        while(store->disk.loaded && disk_saver--) {
            nst_store_memory_sync_disk(tproxy.cache);
...
nst_cache_init() {
    ...
    root       = global.tproxy.cache.root;
    dict_size  = global.tproxy.cache.dict_size;
    data_size  = global.tproxy.cache.data_size;
    size       = dict_size + data_size;
    clean_temp = global.tproxy.cache.clean_temp;

    tproxy.applet.cache.fct = nst_cache_handler;

    if(global.tproxy.cache.status == NST_STATUS_ON) {

        shmem = nst_shmem_create("cache.shm", size, global.tune.bufsize, NST_DEFAULT_CHUNK_SIZE);

        if(!shmem) {
            ha_alert("Failed to create tproxy cache memory zone.\n");
            exit(1);
        }

        global.tproxy.cache.shmem = shmem;

        if(nst_shctx_init(shmem) != NST_OK) {
            ha_alert("Failed to init tproxy cache memory.\n");
            exit(1);
        }

        tproxy.cache = nst_shmem_alloc(shmem, sizeof(nst_core_t));

        if(!tproxy.cache) {
            ha_alert("Failed to init tproxy cache core.\n");
            exit(1);
        }

        memset(tproxy.cache, 0, sizeof(*tproxy.cache));

        tproxy.cache->shmem = shmem;
        tproxy.cache->root  = root;

        if(nst_store_init(&tproxy.cache->store, root, shmem, clean_temp, tproxy.cache) != NST_OK) {
            ha_alert("Failed to init tproxy cache store.\n");
            exit(1);
        }

        if(nst_dict_init(&tproxy.cache->dict, &tproxy.cache->store, shmem, dict_size) != NST_OK) {
            ha_alert("Failed to init tproxy cache dict.\n");
            exit(1);
        }
...
nst_cache_create(hpx_http_msg_t *msg, nst_ctx_t *ctx) {
    ...
    dict = &tproxy.cache->dict;
    mem  = &tproxy.cache->store.memory;
    disk = &tproxy.cache->store.disk;
...
nst_cache_finish(nst_ctx_t *ctx) {
    nst_dict_t        *dict  = &tproxy.cache->dict;
    nst_disk_t        *disk  = &tproxy.cache->store.disk;
    ...
    if(nst_store_memory_on(ctx->rule->prop.store) && ctx->store.memory.obj) {
        ...
        if(entry && entry->state != NST_DICT_ENTRY_STATE_INVALID && entry->store.memory.obj) {
            ...
            nst_memory_incr_invalid(&tproxy.cache->store.memory);
        }
...
nst_cache_exists(nst_ctx_t *ctx) {
    ...
    if(ret == NST_CTX_STATE_INIT) {
        ...
        if(!nst_store_disk_off(ctx->rule->prop.store)) {
            ...
            if(!disk->loaded || global.tproxy.cache.always_check_disk) {
...
nst_cache_abort(nst_ctx_t *ctx) {
    ...
    if(entry->state == NST_DICT_ENTRY_STATE_INIT || entry->state == NST_DICT_ENTRY_STATE_UPDATE) {
        ...
        if(ctx->store.memory.obj) {
            nst_memory_obj_abort(&tproxy.cache->store.memory, ctx->store.memory.obj);
        }

        if(ctx->store.disk.obj.file) {
            nst_disk_obj_abort(&tproxy.cache->store.disk, &ctx->store.disk.obj);
        }
    }
...
nst_cache_delete(nst_key_t *key) {
    ...
    if(entry) {
        ...
        if(entry->state == NST_DICT_ENTRY_STATE_VALID
            ...
            if(entry->store.disk.file) {
                ...
                nst_shmem_free(tproxy.cache->shmem, entry->store.disk.file);
    ...
    if(!tproxy.cache->store.disk.loaded && global.tproxy.cache.root.len){
        nst_disk_obj_t  disk;
        hpx_buffer_t    *buf = get_trash_chunk();

        disk.file = buf->area;

        ret = nst_disk_purge_by_key(&disk, key, global.tproxy.cache.root);
...
nst_cache_hit(hpx_stream_t *s, hpx_stream_interface_t *si, hpx_channel_t *req, hpx_channel_t *res,
    ...
    /*
     * set backend to tproxy.applet.cache
     */
    s->target = &tproxy.applet.cache.obj_type;
    ...
    if(unlikely(!si_register_handler(si, objt_applet(s->target)))) {
        ...
        appctx = si_appctx(si);
        memset(&appctx->ctx.tproxy.store, 0, sizeof(appctx->ctx.tproxy.store));

        appctx->st0 = ctx->state;

        if(ctx->state == NST_CTX_STATE_HIT_MEMORY) {
            nst_memory_obj_attach(&tproxy.cache->store.memory, ctx->store.memory.obj);
            appctx->ctx.tproxy.store.memory.obj  = ctx->store.memory.obj;
            appctx->ctx.tproxy.store.memory.item = ctx->store.memory.obj->item;
        } else {
            char  *meta = ctx->store.disk.obj.meta;

            appctx->ctx.tproxy.store.disk.fd          = ctx->store.disk.obj.fd;
            appctx->ctx.tproxy.store.disk.offset      = nst_disk_pos_header(&ctx->store.disk.obj);
            appctx->ctx.tproxy.store.disk.header_len  = nst_disk_meta_get_header_len(meta);
            appctx->ctx.tproxy.store.disk.payload_len = nst_disk_meta_get_payload_len(meta);
        }
