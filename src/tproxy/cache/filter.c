/*
 * tproxy cache filter related variables and functions.
 *
...
_nst_cache_filter_attach(hpx_stream_t *s, hpx_filter_t *filter) {
    ...
    if(global.tproxy.cache.status != NST_STATUS_ON || conf->status != NST_STATUS_ON) {
        return 0;
    }
...
_nst_cache_filter_detach(hpx_stream_t *s, hpx_filter_t *filter) {
    ...
    if(filter->ctx) {
        ...
        if(ctx->state == NST_CTX_STATE_HIT_MEMORY) {
            nst_memory_obj_detach(&tproxy.cache->store.memory, ctx->store.memory.obj);
        }
