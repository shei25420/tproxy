...
nst_purger_basic(hpx_stream_t *s, hpx_channel_t *req, hpx_proxy_t *px) {
    ...
    while(p) {
        ...
        if(p->tproxy.mode == NST_MODE_CACHE || p->tproxy.mode == NST_MODE_NOSQL) {

            nst_rule_t  *rule = tproxy.proxy[p->uuid]->rule;
...
nst_purger_advanced(hpx_stream_t *s, hpx_channel_t *req, hpx_proxy_t *px) {
    ...
    if(http_find_header(htx, ist("tproxy-host"), &hdr, 0)) {
        host = hdr.value;
...
purge:
    ...
    if(mode == NST_MODE_CACHE && global.tproxy.cache.status == NST_STATUS_OFF) {
        goto err;
    }

    if(mode == NST_MODE_NOSQL && global.tproxy.nosql.status == NST_STATUS_OFF) {
        goto err;
    }

    s->target = &tproxy.applet.purger.obj_type;
    ...
    if(unlikely(!si_register_handler(si, objt_applet(s->target)))) {
        ...
        appctx = si_appctx(si);
        memset(&appctx->ctx.tproxy.manager, 0, sizeof(appctx->ctx.tproxy.manager));

        appctx->st0 = method;

        if(mode == NST_MODE_CACHE) {
            appctx->ctx.tproxy.manager.dict = &tproxy.cache->dict;
        } else {
            appctx->ctx.tproxy.manager.dict = &tproxy.nosql->dict;
        }
        ...
        if(buf.size) {
            buf.area = nst_shmem_alloc(appctx->ctx.tproxy.manager.dict->shmem, buf.size);
        ...
        switch(method) {
            case NST_MANAGER_PROXY:
            case NST_MANAGER_RULE:
                appctx->ctx.tproxy.manager.name = ist2(buf.area + buf.data, name.len);
                chunk_istcat(&buf, name);
                break;
            case NST_MANAGER_PATH:
                appctx->ctx.tproxy.manager.path = ist2(buf.area + buf.data, path.len);
                chunk_istcat(&buf, path);
                break;
            case NST_MANAGER_HOST:
                appctx->ctx.tproxy.manager.host = ist2(buf.area + buf.data, host.len);
                chunk_istcat(&buf, host);
                break;
            case NST_MANAGER_REGEX_HOST:
                appctx->ctx.tproxy.manager.regex = regex;
                appctx->ctx.tproxy.manager.host  = ist2(buf.area + buf.data, host.len);
                chunk_istcat(&buf, host);
                break;
            case NST_MANAGER_PATH_HOST:
        ...
        appctx->ctx.tproxy.manager.buf = buf;
...
nst_purger_check(hpx_appctx_t *appctx, nst_dict_entry_t *entry) {
    ...
    switch(appctx->st0) {
        case NST_MANAGER_PROXY:
            ret = isteq(entry->prop.pid, appctx->ctx.tproxy.manager.name);

            break;
        case NST_MANAGER_RULE:
            ret = isteq(entry->prop.rid, appctx->ctx.tproxy.manager.name);

            break;
        case NST_MANAGER_PATH:
            ret = isteq(entry->path, appctx->ctx.tproxy.manager.path);

            break;
        case NST_MANAGER_REGEX:
            ret = regex_exec2(appctx->ctx.tproxy.manager.regex, entry->path.ptr, entry->path.len);

            break;
        case NST_MANAGER_HOST:
            ret = isteq(entry->host, appctx->ctx.tproxy.manager.host);

            break;
        case NST_MANAGER_PATH_HOST:
            ret = isteq(entry->path, appctx->ctx.tproxy.manager.path)
                && isteq(entry->host, appctx->ctx.tproxy.manager.host);

            break;
        case NST_MANAGER_REGEX_HOST:
            ret = isteq(entry->host, appctx->ctx.tproxy.manager.host)
                && regex_exec2(appctx->ctx.tproxy.manager.regex, entry->path.ptr, entry->path.len);
...
nst_purger_handler(hpx_appctx_t *appctx) {
    ...
    nst_dict_t              *dict   = appctx->ctx.tproxy.manager.dict;
    uint64_t                 start  = nst_time_now_ms();
    int                      max    = 1000;

    while(1) {

        while(appctx->ctx.tproxy.manager.idx < dict->size && max--) {
            nst_shctx_lock(dict);

            entry = dict->entry[appctx->ctx.tproxy.manager.idx];
    ...
    if(appctx->ctx.tproxy.manager.idx == dict->size) {
        nst_http_reply(s, NST_HTTP_200);
    }
}
...
nst_purger_release_handler(hpx_appctx_t *appctx) {
    ...
    if(appctx->ctx.tproxy.manager.regex) {
        regex_free(appctx->ctx.tproxy.manager.regex);
    }

    nst_shmem_free(appctx->ctx.tproxy.manager.dict->shmem, appctx->ctx.tproxy.manager.buf.area);
}

void
nst_purger_init() {
    tproxy.applet.purger.fct     = nst_purger_handler;
    tproxy.applet.purger.release = nst_purger_release_handler;
}
