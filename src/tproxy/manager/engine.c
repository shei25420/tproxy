...
_nst_manager_check_uri(hpx_http_msg_t *msg) {
    ...
    if(!global.tproxy.manager.uri.len) {
        return NST_ERR;
    }
    ...
    if(!isteq(global.tproxy.manager.uri, uri)) {
        return NST_ERR;
    }
...
_nst_manager_check_purge_method(hpx_http_txn_t *txn, hpx_http_msg_t *msg) {
    ...
    if(meth == HTTP_METH_OTHER && isteqi(htx_sl_req_meth(sl), global.tproxy.manager.purge_method)) {
        return NST_OK;
    } else {
        return NST_ERR;
    }
...
nst_manager(hpx_stream_t *s, hpx_channel_t *req, hpx_proxy_t *px) {
    ...
    if(global.tproxy.cache.status == NST_STATUS_OFF
