/*
 * tproxy parser related variables and functions.
 *
...
const char *nst_cache_flt_id = "tproxy cache id";
const char *nst_nosql_flt_id = "tproxy nosql id";
...
int
tproxy_parse_global_manager(const char *file, int line, char **args) {
    int  err_code = 0;
    int  cur_arg  = 1;

    if(global.tproxy.manager.status != NST_STATUS_UNDEFINED) {
    ...
    if(!strcmp(args[cur_arg], "off")) {
        global.tproxy.manager.status = NST_STATUS_OFF;
    } else if(!strcmp(args[cur_arg], "on")) {
        global.tproxy.manager.status = NST_STATUS_ON;
    } else {
    ...
    global.tproxy.manager.purge_method = ist(NST_MANAGER_DEFAULT_PURGE_METHOD);
    global.tproxy.manager.uri          = ist(NST_MANAGER_DEFAULT_URI);
    ...
    while(*(args[cur_arg]) !=0) {
        ...
        if(!strcmp(args[cur_arg], "purge-method")) {
            ...
            global.tproxy.manager.purge_method.ptr = strdup(args[cur_arg]);
            global.tproxy.manager.purge_method.len = strlen(args[cur_arg]);
        ...
        if(!strcmp(args[cur_arg], "uri")) {
            ...
            global.tproxy.manager.uri.ptr = strdup(args[cur_arg]);
            global.tproxy.manager.uri.len = strlen(args[cur_arg]);
...
int
tproxy_parse_global_cache(const char *file, int line, char **args) {
    int  err_code = 0;
    int  cur_arg  = 1;

    if(global.tproxy.cache.status != NST_STATUS_UNDEFINED) {
        ha_warning("parsing [%s:%d]: [%s] already specified. Ignore.\n", file, line, args[0]);
    ...
    if(!strcmp(args[cur_arg], "off")) {
        global.tproxy.cache.status = NST_STATUS_OFF;
    } else if(!strcmp(args[cur_arg], "on")) {
        global.tproxy.cache.status = NST_STATUS_ON;
    } else {
        ha_alert("parsing [%s:%d]: [%s] only supports 'on' and 'off'.\n", file, line, args[0]);
    ...
    while(*(args[cur_arg]) !=0) {
        ...
        if(!strcmp(args[cur_arg], "dir")) {
            ...
            global.tproxy.cache.root.ptr = strdup(args[cur_arg]);
            global.tproxy.cache.root.len = strlen(args[cur_arg]);
        ...
        if(!strcmp(args[cur_arg], "dict-cleaner")) {
            ...
            global.tproxy.cache.dict_cleaner = atoi(args[cur_arg]);

            if(global.tproxy.cache.dict_cleaner <= 0) {
                global.tproxy.cache.dict_cleaner = NST_DEFAULT_DICT_CLEANER;
            }
        ...
        if(!strcmp(args[cur_arg], "data-cleaner")) {
            ...
            global.tproxy.cache.data_cleaner = atoi(args[cur_arg]);

            if(global.tproxy.cache.data_cleaner <= 0) {
                global.tproxy.cache.data_cleaner = NST_DEFAULT_DATA_CLEANER;
            }
        ...
        if(!strcmp(args[cur_arg], "disk-cleaner")) {
            ...
            global.tproxy.cache.disk_cleaner = atoi(args[cur_arg]);

            if(global.tproxy.cache.disk_cleaner <= 0) {
                global.tproxy.cache.disk_cleaner = NST_DEFAULT_DISK_CLEANER;
            }
        ...
        if(!strcmp(args[cur_arg], "disk-loader")) {
            ...
            global.tproxy.cache.disk_loader = atoi(args[cur_arg]);

            if(global.tproxy.cache.disk_loader <= 0) {
                global.tproxy.cache.disk_loader = NST_DEFAULT_DISK_LOADER;
            }
        ...
        if(!strcmp(args[cur_arg], "disk-saver")) {
            ...
            global.tproxy.cache.disk_saver = atoi(args[cur_arg]);

            if(global.tproxy.cache.disk_saver <= 0) {
                global.tproxy.cache.disk_saver = NST_DEFAULT_DISK_SAVER;
            }
        ...
        if(!strcmp(args[cur_arg], "clean-temp")) {
            ...
            if(!strcmp(args[cur_arg], "off")) {
                global.tproxy.cache.clean_temp = NST_STATUS_OFF;
            } else if(!strcmp(args[cur_arg], "on")) {
                global.tproxy.cache.clean_temp = NST_STATUS_ON;
            } else {
                ha_alert("parsing [%s:%d]: [%s] only supports 'on' and 'off'.\n", file, line,
                        args[0]);
        ...
        if(!strcmp(args[cur_arg], "always-check-disk")) {
            ...
            if(!strcmp(args[cur_arg], "off")) {
                global.tproxy.cache.always_check_disk = NST_STATUS_OFF;
            } else if(!strcmp(args[cur_arg], "on")) {
                global.tproxy.cache.always_check_disk = NST_STATUS_ON;
            } else {
                ha_alert("parsing [%s:%d]: [%s] only supports 'on' and 'off'.\n", file, line,
                        args[0]);
...
int
tproxy_parse_global_nosql(const char *file, int line, char **args) {
    int  err_code = 0;
    int  cur_arg  = 1;

    if(global.tproxy.nosql.status != NST_STATUS_UNDEFINED) {
        ha_warning("parsing [%s:%d]: [%s] already specified. Ignore.\n", file, line, args[0]);
    ...
    if(!strcmp(args[cur_arg], "off")) {
        global.tproxy.nosql.status = NST_STATUS_OFF;
    } else if(!strcmp(args[cur_arg], "on")) {
        global.tproxy.nosql.status = NST_STATUS_ON;
    } else {
        ha_alert("parsing [%s:%d]: [%s] only supports 'on' and 'off'.\n", file, line, args[0]);
    ...
    while(*(args[cur_arg]) !=0) {
        ...
        if(!strcmp(args[cur_arg], "dict-size")) {
            ...
            if(nst_parse_size(args[cur_arg], &global.tproxy.nosql.dict_size)) {
        ...
        if(!strcmp(args[cur_arg], "data-size")) {
            ...
            if(nst_parse_size(args[cur_arg], &global.tproxy.nosql.data_size)) {
        ...
        if(!strcmp(args[cur_arg], "dir")) {
            ...
            global.tproxy.nosql.root.ptr = strdup(args[cur_arg]);
            global.tproxy.nosql.root.len = strlen(args[cur_arg]);
        ...
        if(!strcmp(args[cur_arg], "dict-cleaner")) {
            ...
            global.tproxy.nosql.dict_cleaner = atoi(args[cur_arg]);

            if(global.tproxy.nosql.dict_cleaner <= 0) {
                global.tproxy.nosql.dict_cleaner = NST_DEFAULT_DICT_CLEANER;
            }
        ...
        if(!strcmp(args[cur_arg], "data-cleaner")) {
            ...
            global.tproxy.nosql.data_cleaner = atoi(args[cur_arg]);

            if(global.tproxy.nosql.data_cleaner <= 0) {
                global.tproxy.nosql.data_cleaner = NST_DEFAULT_DATA_CLEANER;
            }
        ...
        if(!strcmp(args[cur_arg], "disk-cleaner")) {
            ...
            global.tproxy.nosql.disk_cleaner = atoi(args[cur_arg]);

            if(global.tproxy.nosql.disk_cleaner <= 0) {
                global.tproxy.nosql.disk_cleaner = NST_DEFAULT_DISK_CLEANER;
            }
        ...
        if(!strcmp(args[cur_arg], "disk-loader")) {
            ...
            global.tproxy.nosql.disk_loader = atoi(args[cur_arg]);

            if(global.tproxy.nosql.disk_loader <= 0) {
                global.tproxy.nosql.disk_loader = NST_DEFAULT_DISK_LOADER;
            }
        ...
        if(!strcmp(args[cur_arg], "disk-saver")) {
            ...
            global.tproxy.nosql.disk_saver = atoi(args[cur_arg]);

            if(global.tproxy.nosql.disk_saver <= 0) {
                global.tproxy.nosql.disk_saver = NST_DEFAULT_DISK_SAVER;
            }
        ...
        if(!strcmp(args[cur_arg], "clean-temp")) {
            ...
            if(!strcmp(args[cur_arg], "off")) {
                global.tproxy.nosql.clean_temp = NST_STATUS_OFF;
            } else if(!strcmp(args[cur_arg], "on")) {
                global.tproxy.nosql.clean_temp = NST_STATUS_ON;
            } else {
                ha_alert("parsing [%s:%d]: [%s] only supports 'on' and 'off'.\n", file, line,
                        args[0]);
        ...
        if(!strcmp(args[cur_arg], "always-check-disk")) {
            ...
            if(!strcmp(args[cur_arg], "off")) {
                global.tproxy.nosql.always_check_disk = NST_STATUS_OFF;
            } else if(!strcmp(args[cur_arg], "on")) {
                global.tproxy.nosql.always_check_disk = NST_STATUS_ON;
            } else {
                ha_alert("parsing [%s:%d]: [%s] only supports 'on' and 'off'.\n", file, line,
                        args[0]);
...
nst_parse_proxy_rule(char **args, int section, hpx_proxy_t *proxy, hpx_proxy_t *defpx,
    ...
    if(disk == NST_STORE_DISK_ON || disk == NST_STORE_DISK_SYNC) {
        if((proxy->tproxy.mode == NST_MODE_CACHE && !global.tproxy.cache.root.len)
                || (proxy->tproxy.mode == NST_MODE_NOSQL && !global.tproxy.nosql.root.len)) {
    ...
    LIST_INIT(&rule->list);
    LIST_ADDQ(&proxy->tproxy.rules, &rule->list);
...
static struct cfg_kw_list cfg_kws = {ILH, {
    { CFG_LISTEN, "tproxy", nst_parse_proxy}, { 0, NULL, NULL }, }
};
