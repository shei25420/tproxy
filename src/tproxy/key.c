...
nst_key_debug(hpx_stream_t *s, nst_key_t *key) {
    ...
    if((global.mode & MODE_DEBUG)) {
        ...
        chunk_printf(&trash, "%08x:%s.tproxy[%04x:%04x]: [key  ] raw:  |%d| ",
                s->uniq_id, s->be->id,
        ...
        chunk_appendf(&trash, "%08x:%s.tproxy[%04x:%04x]: [key  ] hash: %"PRIu64"\n",
                s->uniq_id, s->be->id,
                objt_conn(sess->origin) ?
                (unsigned short)objt_conn(sess->origin)->handle.fd : -1,
                objt_cs(s->si[1].end) ?
        ...
        chunk_appendf(&trash, "%08x:%s.tproxy[%04x:%04x]: [key  ] uuid: ",
                s->uniq_id, s->be->id,
                objt_conn(sess->origin) ?
                (unsigned short)objt_conn(sess->origin)->handle.fd : -1,
                objt_cs(s->si[1].end) ?
