...
int cfg_parse_global(const char *file, int linenum, char **args, int kwm)
{
	...
	/* end of user/group name handling*/
		...
		if (!strcmp(args[cur_arg], "cache")) {
			...
			args++;
			err_code = tproxy_parse_global_cache(file, linenum, args);
			if (err_code) {
				goto out;
			}
			...
			args++;
			err_code = tproxy_parse_global_nosql(file, linenum, args);
			if (err_code) {
				goto out;
			}
			...
			args++;
			err_code = tproxy_parse_global_manager(file, linenum, args);
			if (err_code) {
				goto out;
			}
		} else {
			ha_alert("parsing [%s:%d] : [global] '%s' only supports 'cache|nosql|manager' .\n", file, linenum, args[0]);
			err_code |= ERR_ALERT | ERR_FATAL;
			goto out;
		}
	}
