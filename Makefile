...
ifeq ($(USE_OPENSSL),)
ifneq ($(USE_PTHREAD_PSHARED),)
OPTIONS_CFLAGS  += -Dtproxy_USE_PTHREAD
OPTIONS_LDFLAGS += -lpthread
endif
endif
ifneq ($(USE_LUA),)
...
OBJS += src/tproxy/cache/engine.o src/tproxy/cache/filter.o                    \
        src/tproxy/nosql/engine.o src/tproxy/nosql/filter.o                    \
        src/tproxy/manager/stats.o src/tproxy/manager/engine.o                 \
        src/tproxy/manager/purger.o                                            \
        src/tproxy/store/memory.o src/tproxy/store/disk.o                      \
        src/tproxy/shmem.o src/tproxy/parser.o src/tproxy/http.o               \
        src/tproxy/key.o src/tproxy/dict.o src/tproxy/sample.o                 \
        src/tproxy/misc.o src/tproxy/tproxy.o
ifneq ($(TRACE),)
...
install-bin:
	...
	$(Q)ln -s "$(DESTDIR)$(SBINDIR)/haproxy" "$(DESTDIR)$(SBINDIR)/tproxy"
...
clean:
	...
	$(Q)rm -f src/tproxy/*.[oas] src/tproxy/*/*.[oas]
